...
static void rtc_irq_handler(void)
{
	...
	if (irqn == SYS_RTC_IRQn)
		alarm_cb();
	else if (irqn == RTC_PERIODIC_IRQ)
		periodic_cb();
	else if (irqn == RTC_2HZ_IRQn)
		rtc2hz_cb();
}
...
void HAL_RTC_Init(uint32_t sec, IRQHandler_t cb_alarm, IRQHandler_t cb_periodic, IRQHandler_t cb_2hz)
{
	...
	if(cb_alarm != NULL) {
		alarm_cb = cb_alarm;
		IRQ_SetHandler(SYS_RTC_IRQn, rtc_irq_handler);
		IRQ_Enable(SYS_RTC_IRQn);
		...
		rtc2hz_cb = cb_2hz;
		IRQ_SetHandler(RTC_2HZ_IRQn, rtc_irq_handler);
		IRQ_Enable(RTC_2HZ_IRQn);
	}
...
void HAL_RTC_DeInit(void)
{
	...
	IRQ_Disable(SYS_RTC_IRQn);
	IRQ_Disable(RTC_PERIODIC_IRQ);
	IRQ_Disable(RTC_2HZ_IRQn);
}
