...
/*------------------------------------------------------------------------*/
int CU_get_elapsed_time(void)
{
  if (CU_TRUE == f_bTestIsRunning) {
    return ((int)HAL_GetTick() - (int)f_start_time)/(int)CLOCKS_PER_SEC;
  }
  else {
...
CU_ErrorCode CU_run_all_tests(void)
{
  ...
  if (NULL == pRegistry) {
    ...
    f_run_summary.ElapsedTime = ((int)HAL_GetTick() - (int)f_start_time)/(int)CLOCKS_PER_SEC;
...
CU_EXPORT void CU_print_run_results(FILE *file)
{
  ...
  if (NULL != summary_string) {
	...
	printf(_("An error occurred printing the run results."));
  }
}
...

{
  ...
  if (NULL != result) {
    ...
    snprintf(result, len, "%*s%*s%*s%*s%*s%*s%*s\n"   /* if you change this, be sure  */
            ...
            width[6], _("n/a"),
            width[7], _("Elapsed time = "), (int)(CU_get_elapsed_time()/1000),  /* makes sure time is updated */
			width[8], _(" seconds")
            );
     result[len-1] = '\0';
  }
                           ...
                           CU_pTest ptest, CU_pFailureRecord pfailure)
{
  ...
  if (NULL == pNewEvent) {
	printf("Memory allocation failed in add_test_event().");    
	exit(1);
  }
